...
public class Ballistica {
	...
	private void build( int from, int to, boolean stopTarget, boolean stopChars, boolean stopTerrain ) {
		int w = com.quasistellar.shatteredpixeldungeon.Dungeon.level.width();
		...
		int err = dA / 2;
		while (com.quasistellar.shatteredpixeldungeon.Dungeon.level.insideMap(cell)) {
			...
			if (stopTerrain && cell != sourcePos && !com.quasistellar.shatteredpixeldungeon.Dungeon.level.passable[cell] && !com.quasistellar.shatteredpixeldungeon.Dungeon.level.avoid[cell]) {
