...
package com.quasistellar.shatteredpixeldungeon.mechanics;
...
public class ConeAOE {
	...
	public ConeAOE( int from, int to, float maxDist, float degrees, int ballisticaParams ){
		...
		PointF fromP = new PointF(com.quasistellar.shatteredpixeldungeon.Dungeon.level.cellToPoint(from));
		fromP.x += 0.5f;
		fromP.y += 0.5f;
		PointF toP = new PointF(com.quasistellar.shatteredpixeldungeon.Dungeon.level.cellToPoint(to));
		toP.x += 0.5f;
		toP.y += 0.5f;
		...
		for (float a = initalAngle+degrees/2f; a >= initalAngle-degrees/2f; a-=0.5f){
			...
			scanInt.set(
					(int)GameMath.gate(0, (int)Math.floor(scan.x), com.quasistellar.shatteredpixeldungeon.Dungeon.level.width()-1),
					(int)GameMath.gate(0, (int)Math.floor(scan.y), com.quasistellar.shatteredpixeldungeon.Dungeon.level.height()-1));
			targetCells.add(com.quasistellar.shatteredpixeldungeon.Dungeon.level.pointToCell(scanInt));
