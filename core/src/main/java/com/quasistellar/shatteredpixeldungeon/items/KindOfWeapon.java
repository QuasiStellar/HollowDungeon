...
abstract public class KindOfWeapon extends EquipableItem {
	...
	public int damageRoll( com.quasistellar.shatteredpixeldungeon.actors.Char owner ) {
		return Random.NormalIntRange( min(), max() );
	}
	
	public float accuracyFactor( com.quasistellar.shatteredpixeldungeon.actors.Char owner ) {
		return 1f;
	}
	
	public float speedFactor( com.quasistellar.shatteredpixeldungeon.actors.Char owner ) {
		return 1f;
	}

	public int reachFactor( com.quasistellar.shatteredpixeldungeon.actors.Char owner ){
		return 1;
	}
	
	public boolean canReach(com.quasistellar.shatteredpixeldungeon.actors.Char owner, int target){
		if (Dungeon.level.distance( owner.pos, target ) > reachFactor(owner)){
			return false;
		} else {
			boolean[] passable = BArray.not(Dungeon.level.solid, null);
			for (com.quasistellar.shatteredpixeldungeon.actors.Char ch : Actor.chars()) {
	...
	public int defenseFactor( com.quasistellar.shatteredpixeldungeon.actors.Char owner ) {
		return 0;
	}
	
	public int proc(com.quasistellar.shatteredpixeldungeon.actors.Char attacker, Char defender, int damage ) {
		return damage;
	}
