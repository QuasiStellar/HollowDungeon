...
import com.quasistellar.shatteredpixeldungeon.tiles.CustomTilemap;
...
public class WeakFloorRoom extends SpecialRoom {
	...
	public void paint( Level level ) {
		...
		Painter.fill( level, this, 1, com.quasistellar.shatteredpixeldungeon.levels.Terrain.CHASM );
		...
		if (door.x == left) {
			for (int i=top + 1; i < bottom; i++) {
				Painter.drawInside( level, this, new Point( left, i ), Random.IntRange( 1, width() - 4 ), com.quasistellar.shatteredpixeldungeon.levels.Terrain.EMPTY_SP );
			}
			well = new Point( right-1, Random.Int( 2 ) == 0 ? top + 2 : bottom - 1 );
		} else if (door.x == right) {
			for (int i=top + 1; i < bottom; i++) {
				Painter.drawInside( level, this, new Point( right, i ), Random.IntRange( 1, width() - 4 ), com.quasistellar.shatteredpixeldungeon.levels.Terrain.EMPTY_SP );
			}
			well = new Point( left+1, Random.Int( 2 ) == 0 ? top + 2 : bottom - 1 );
		} else if (door.y == top) {
			for (int i=left + 1; i < right; i++) {
				Painter.drawInside( level, this, new Point( i, top ), Random.IntRange( 1, height() - 4 ), com.quasistellar.shatteredpixeldungeon.levels.Terrain.EMPTY_SP );
			}
			well = new Point( Random.Int( 2 ) == 0 ? left + 1 : right - 1, bottom-1 );
		} else if (door.y == bottom) {
			for (int i=left + 1; i < right; i++) {
				Painter.drawInside( level, this, new Point( i, bottom ), Random.IntRange( 1, height() - 4 ), com.quasistellar.shatteredpixeldungeon.levels.Terrain.EMPTY_SP );
			}
			well = new Point( Random.Int( 2 ) == 0 ? left + 1 : right - 1, top+2 );
		}
		
		Painter.set(level, well, Terrain.CHASM);
		com.quasistellar.shatteredpixeldungeon.tiles.CustomTilemap vis = new HiddenWell();
