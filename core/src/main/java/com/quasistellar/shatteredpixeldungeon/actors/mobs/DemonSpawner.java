...
import com.quasistellar.shatteredpixeldungeon.sprites.SpawnerSprite;
...
public class DemonSpawner extends Mob {
	...
	protected boolean act() {
		if (!spawnRecorded){
			com.quasistellar.shatteredpixeldungeon.Statistics.spawnersAlive++;
			spawnRecorded = true;
		}
		...
		if (spawnCooldown <= 0){
			...
			for (int n : PathFinder.NEIGHBOURS8) {
				if (com.quasistellar.shatteredpixeldungeon.Dungeon.level.passable[pos+n] && com.quasistellar.shatteredpixeldungeon.actors.Actor.findChar( pos+n ) == null) {
			...
			if (!candidates.isEmpty()) {
				...
				com.quasistellar.shatteredpixeldungeon.Dungeon.level.occupyCell(spawn);
				...
				spawnCooldown += 60;
				if (com.quasistellar.shatteredpixeldungeon.Dungeon.depth > 21){
