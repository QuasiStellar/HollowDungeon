...
import com.quasistellar.shatteredpixeldungeon.items.food.FrozenCarpaccio;
import com.quasistellar.shatteredpixeldungeon.items.food.MysteryMeat;
...
public class Frost extends FlavourBuff {
	...
	public boolean attachTo( Char target ) {
		if (super.attachTo( target )) {
			...
			target.paralysed++;
			com.quasistellar.shatteredpixeldungeon.actors.buffs.Buff.detach( target, Burning.class );
			com.quasistellar.shatteredpixeldungeon.actors.buffs.Buff.detach( target, com.quasistellar.shatteredpixeldungeon.actors.buffs.Chill.class );

			if (target instanceof Hero) {

				Hero hero = (Hero)target;
				ArrayList<com.quasistellar.shatteredpixeldungeon.items.Item> freezable = new ArrayList<>();
				//does not reach inside of containers
				for (com.quasistellar.shatteredpixeldungeon.items.Item i : hero.belongings.backpack.items){
				...
				if (!freezable.isEmpty()){
					com.quasistellar.shatteredpixeldungeon.items.Item toFreeze = Random.element(freezable).detach( hero.belongings.backpack );
					...
					if (toFreeze instanceof Potion){
						...
						if (!carpaccio.collect( hero.belongings.backpack )) {
							com.quasistellar.shatteredpixeldungeon.Dungeon.level.drop( carpaccio, target.pos ).sprite.drop();
						}
					}
				}
	...
	public void detach() {
		...
		if (Dungeon.level.water[target.pos])
			Buff.prolong(target, com.quasistellar.shatteredpixeldungeon.actors.buffs.Chill.class, Chill.DURATION/2f);
	}
	...
	public void fx(boolean on) {
		if (on) target.sprite.add(com.quasistellar.shatteredpixeldungeon.sprites.CharSprite.State.FROZEN);
