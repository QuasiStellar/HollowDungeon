...
import com.quasistellar.shatteredpixeldungeon.actors.buffs.Paralysis;
...
public class StenchGas extends Blob {
			...
			for (int j = area.top; j < area.bottom; j++){
				...
				if (cur[cell] > 0 && (ch = Actor.findChar( cell )) != null) {
					if (!ch.isImmune(this.getClass()))
						Buff.prolong( ch, com.quasistellar.shatteredpixeldungeon.actors.buffs.Paralysis.class, Paralysis.DURATION/5 );
				}
			}
		}
	}
	...
	public void use( BlobEmitter emitter ) {
		...
		emitter.pour( com.quasistellar.shatteredpixeldungeon.effects.Speck.factory(Speck.STENCH), 0.4f );
	}
