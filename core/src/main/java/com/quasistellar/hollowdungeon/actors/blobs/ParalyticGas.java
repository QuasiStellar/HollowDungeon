...
public class ParalyticGas extends Blob {
	...
	protected void evolve() {
		...
		for (int i = area.left; i < area.right; i++) {
			for (int j = area.top; j < area.bottom; j++) {
				...
				if (cur[cell] > 0 && (ch = com.quasistellar.hollowdungeon.actors.Actor.findChar(cell)) != null) {
					if (!ch.isImmune(this.getClass()))
						Buff.prolong(ch, com.quasistellar.hollowdungeon.actors.buffs.Paralysis.class, Paralysis.DURATION);
				}
			}
		}
	}
	...
	public void use( BlobEmitter emitter ) {
		...
		emitter.pour( Speck.factory( com.quasistellar.hollowdungeon.effects.Speck.PARALYSIS ), 0.4f );
	}
