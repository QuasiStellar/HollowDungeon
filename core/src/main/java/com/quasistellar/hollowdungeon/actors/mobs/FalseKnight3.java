...
public class FalseKnight3 extends Mob {
    ...
    public void die(Object cause) {
        super.die(cause);
        FalseKnightMaggot3 falseKnightMaggot3;
        falseKnightMaggot3 = new FalseKnightMaggot3();
        falseKnightMaggot3.pos = this.pos;
        GameScene.add( falseKnightMaggot3 );
        Dungeon.level.occupyCell( falseKnightMaggot3 );
    }
    ...
    public class Hunting extends Mob.Hunting {
        ...
        public boolean act( boolean enemyInFOV, boolean justAlerted ) {
            ...
            if (leapPos != -1){
                ...
                sprite.jump(pos, leapPos, new Callback() {
                    ...
                    public void call() {
                        ...
                        if (endPos != leapPos){
                            Actor.addDelayed(new Pushing(FalseKnight3.this, leapPos, endPos), -1);
                        }

                        pos = endPos;
                        leapPos = -1;
                        sprite.idle();
                        Dungeon.level.occupyCell(FalseKnight3.this);

                        spend(TICK);

                        throwFire(FalseKnight3.this, Dungeon.hero);
            ...
            if (leapCooldown <= 0 && enemyInFOV && Dungeon.level.distance(pos, enemy.pos) >= 2) {
                ...
                if (b.collisionPos == targetPos){
                    ...
                    if (Dungeon.level.heroFOV[pos] || Dungeon.level.heroFOV[leapPos]){
                        GLog.w(Messages.get(FalseKnight3.this, "leap"));
            ...
            if (enemyInFOV) {
                ...
                state = WANDERING;
                target = Dungeon.level.randomDestination( FalseKnight3.this );
                return true;
            }
    ...
    public static boolean throwFire(final Char thrower, final Char target){
        ...
        for (int i = 0; i < PathFinder.CIRCLE8.length; i++){
            if (aim.sourcePos+PathFinder.CIRCLE8[i] == aim.path.get(1)){
                ...
                Buff.append(thrower, FalseKnight3.FireAbility.class).direction = i;
        ...
        public boolean act() {
            ...
            if (curCells == null){
                ...
                for (Integer c : curCells) {
                    if (FalseKnight3.FireAbility.FireBlob.volumeAt(c, FalseKnight3.FireAbility.FireBlob.class) > 0) spreadFromCell(c);
                }
            }

            for (Integer c : curCells){
                Char ch = Actor.findChar( c );
                if (ch != null && !ch.isImmune(Fire.class) && !(ch instanceof FalseKnight3)) {
                    ch.damage(1, FalseKnight3.class );
                }
                toCells.remove(c);
            }
            ...
            if (toCells.isEmpty()){
                ...
                for (Integer c : toCells){
                    GameScene.add(Blob.seed(c, 2, FalseKnight3.FireAbility.FireBlob.class));
                    curCells[i] = c;
                    i++;
                }
            }
