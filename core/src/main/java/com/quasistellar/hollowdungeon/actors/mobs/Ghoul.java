...
import com.quasistellar.hollowdungeon.sprites.GhoulSprite;
...
public class Ghoul extends Mob {
	...
	{
		spriteClass = com.quasistellar.hollowdungeon.sprites.GhoulSprite.class;
	...
	protected boolean act() {
		...
		if (partnerID == -1){
			...
			if (!candidates.isEmpty()){
				...
				GameScene.add( child );
				if (sprite.visible) {
					Actor.addDelayed( new com.quasistellar.hollowdungeon.effects.Pushing( child, pos, child.pos ), -1 );
				}
			}
	...
	public void die(Object cause) {
		if (cause != Chasm.class && cause != GhoulLifeLink.class && !Dungeon.level.pit[pos]){
			...
			if (nearby != null){
				...
				((com.quasistellar.hollowdungeon.sprites.GhoulSprite)sprite).crumple();
	...
	protected synchronized void onRemove() {
		if (beingLifeLinked) {
			for (com.quasistellar.hollowdungeon.actors.buffs.Buff buff : buffs()) {
	...
	public static class GhoulLifeLink extends com.quasistellar.hollowdungeon.actors.buffs.Buff {
		...
		public void fx(boolean on) {
			if (on && ghoul != null && ghoul.sprite == null){
				com.quasistellar.hollowdungeon.scenes.GameScene.addSprite(ghoul);
		...
		public static Ghoul searchForHost(Ghoul dieing){
			...
			for (Char ch : com.quasistellar.hollowdungeon.actors.Actor.chars()){
				if (ch != dieing && ch instanceof Ghoul && ch.alignment == dieing.alignment){
					...
					if (ch.fieldOfView[dieing.pos] || com.quasistellar.hollowdungeon.Dungeon.level.distance(ch.pos, dieing.pos) < 4){
						return (Ghoul) ch;
					}
				}
			}
