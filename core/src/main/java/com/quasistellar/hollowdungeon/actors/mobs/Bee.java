...
import com.quasistellar.hollowdungeon.sprites.BeeSprite;
...
public class Bee extends Mob {
	...
	public void setPotInfo(int potPos, com.quasistellar.hollowdungeon.actors.Char potHolder){
		this.potPos = potPos;
		if (potHolder == null)
			this.potHolder = -1;
		else
	...
	public void add(Buff buff) {
		super.add(buff);
		if (buff instanceof com.quasistellar.hollowdungeon.actors.buffs.Corruption){
	...
	protected com.quasistellar.hollowdungeon.actors.Char chooseEnemy() {
		...
		//if something is holding the pot, target that
			...
			return (com.quasistellar.hollowdungeon.actors.Char) Actor.findById(potHolder);
	...
	protected boolean getCloser(int target) {
		if (alignment == Alignment.ALLY && enemy == null && buff(com.quasistellar.hollowdungeon.actors.buffs.Corruption.class) == null){
			target = Dungeon.hero.pos;
		} else if (enemy != null && com.quasistellar.hollowdungeon.actors.Actor.findById(potHolder) == enemy) {
			target = enemy.pos;
		} else if (potPos != -1 && (state == WANDERING || com.quasistellar.hollowdungeon.Dungeon.level.distance(target, potPos) > 3))
