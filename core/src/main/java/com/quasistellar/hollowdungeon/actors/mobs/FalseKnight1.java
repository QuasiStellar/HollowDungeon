...
public class FalseKnight1 extends Mob {
    ...
    public void die(Object cause) {
        super.die(cause);
        FalseKnightMaggot1 falseKnightMaggot1;
        falseKnightMaggot1 = new FalseKnightMaggot1();
        falseKnightMaggot1.pos = this.pos;
        GameScene.add( falseKnightMaggot1 );
        Dungeon.level.occupyCell( falseKnightMaggot1 );
    }
    ...
    public class Hunting extends Mob.Hunting {
        ...
        public boolean act( boolean enemyInFOV, boolean justAlerted ) {
            ...
            if (leapPos != -1){
                ...
                sprite.jump(pos, leapPos, new Callback() {
                    ...
                    public void call() {
                        ...
                        if (endPos != leapPos){
                            Actor.addDelayed(new Pushing(FalseKnight1.this, leapPos, endPos), -1);
                        }

                        pos = endPos;
                        leapPos = -1;
                        sprite.idle();
                        Dungeon.level.occupyCell(FalseKnight1.this);

                        spend(3 * TICK);

                        throwFire(FalseKnight1.this, Dungeon.hero);
            ...
            if (leapCooldown <= 0 && enemyInFOV && Dungeon.level.distance(pos, enemy.pos) >= 2) {
                ...
                if (b.collisionPos == targetPos){
                    ...
                    if (Dungeon.level.heroFOV[pos] || Dungeon.level.heroFOV[leapPos]){
                        GLog.w(Messages.get(FalseKnight1.this, "leap"));
            ...
            if (enemyInFOV) {
                ...
                state = WANDERING;
                target = Dungeon.level.randomDestination( FalseKnight1.this );
                return true;
            }
    ...
    public static boolean throwFire(final Char thrower, final Char target){
        ...
        for (int i = 0; i < PathFinder.CIRCLE8.length; i++){
            if (aim.sourcePos+PathFinder.CIRCLE8[i] == aim.path.get(1)){
                ...
                Buff.append(thrower, FalseKnight1.FireAbility.class).direction = i;
                ...
                for (int i = area.left; i < area.right; i++){
                    for (int j = area.top; j < area.bottom; j++){
                        ...
                        if (cur[cell] > 0 && off[cell] == 0) {
                            ...
                            if (ch != null && !ch.isImmune(Fire.class) && !(ch instanceof FalseKnight1)) {
                                ch.damage(1, FalseKnight1.class );
                            }
