...
import com.quasistellar.hollowdungeon.sprites.GolemSprite;
...
public class Golem extends Mob {
	...
	{
		spriteClass = com.quasistellar.hollowdungeon.sprites.GolemSprite.class;
	...
	protected boolean act() {
		...
		enemyTeleCooldown--;
		if (teleporting){
			((com.quasistellar.hollowdungeon.sprites.GolemSprite)sprite).teleParticles(false);
	...
	public void teleportEnemy(){
		...
		for (int i : PathFinder.NEIGHBOURS8){
			if (Dungeon.level.passable[pos + i]
				&& com.quasistellar.hollowdungeon.actors.Actor.findChar(pos+i) == null
	...
	private class Wandering extends Mob.Wandering{
		...
		protected boolean continueWandering() {
			...
			if (target != -1 && getCloser( target )) {
				...
				teleporting = true;
				spend( 2* com.quasistellar.hollowdungeon.actors.Actor.TICK );
			} else {
				target = com.quasistellar.hollowdungeon.Dungeon.level.randomDestination( Golem.this );
				spend( com.quasistellar.hollowdungeon.actors.Actor.TICK );
			}
	...
	private class Hunting extends Mob.Hunting{
		...
		public boolean act(boolean enemyInFOV, boolean justAlerted) {
			if (!enemyInFOV || canAttack(enemy)) {
				...
				} else if (enemyTeleCooldown <= 0 && !com.quasistellar.hollowdungeon.actors.Char.hasProp(enemy, Property.IMMOVABLE)) {
					if (sprite != null && (sprite.visible || enemy.sprite.visible)) {
						sprite.zap( enemy.pos );
						return false;
					} else {
