...
import com.quasistellar.hollowdungeon.sprites.SpawnerSprite;
	...
	protected boolean act() {
		...
		if (spawnCooldown <= 0){
			...
			if (!candidates.isEmpty()) {
				...
				if (sprite.visible) {
					com.quasistellar.hollowdungeon.actors.Actor.addDelayed(new Pushing(spawn, pos, spawn.pos), -1);
				}

				spawnCooldown += 60;
//				if (Dungeon.depth > 21){
//					//60/53.33/46.67/40 turns to spawn on floor 21/22/23/24
//					spawnCooldown -= Math.min(20, (com.quasistellar.hollowdungeon.Dungeon.depth-21)*6.67);
//				}
			}
		}
		return super.act();
	...
	public void die(Object cause) {
		if (spawnRecorded){
			com.quasistellar.hollowdungeon.Statistics.spawnersAlive--;
		}
