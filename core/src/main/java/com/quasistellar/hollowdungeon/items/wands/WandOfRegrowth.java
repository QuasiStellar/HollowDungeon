...
import com.quasistellar.hollowdungeon.items.weapon.enchantments.Blooming;
...
import com.quasistellar.hollowdungeon.sprites.LotusSprite;
...
public class WandOfRegrowth extends Wand {
	...
	protected void onZap( Ballistica bolt ) {
		...
		int overLimit = totChrgUsed - chargeLimit(Dungeon.depth);
		float furrowedChance = overLimit > 0 ? (overLimit / (10f + Dungeon.depth)) : 0;
		...
		if (chargesPerCast() >= 3){
			...
			if (cells.contains(target) && Actor.findChar(target) == null){
				...
				for (int i = bolt.path.size()-1; i >= 0; i--){
					...
					if (cells.contains(c) && com.quasistellar.hollowdungeon.actors.Actor.findChar(c) == null){
		...
		if (!cells.isEmpty() && Random.Float() > furrowedChance && chrgUsed == 3){
			int cell = cells.remove(0);
			Dungeon.level.plant((com.quasistellar.hollowdungeon.plants.Plant.Seed) Generator.randomUsingDefaults(Generator.Category.SEED), cell);
		}
		...
		for (int cell : cells){
			...
			if (Random.Float() > furrowedChance) {
				...
				com.quasistellar.hollowdungeon.levels.Level.set(cell, com.quasistellar.hollowdungeon.levels.Terrain.FURROWED_GRASS);
			}
			GameScene.updateMap( cell );
			grassToPlace--;
		}
	...
	protected void fx( Ballistica bolt, Callback callback ) {
		...
		MagicMissile.boltFromChar( com.quasistellar.hollowdungeon.items.Item.curUser.sprite.parent,
				com.quasistellar.hollowdungeon.effects.MagicMissile.FOLIAGE_CONE,
				Item.curUser.sprite,
	...
	public static class Dewcatcher extends com.quasistellar.hollowdungeon.plants.Plant {
		...
		public static class Seed extends com.quasistellar.hollowdungeon.plants.Plant.Seed {
			{
	...
	public static class Seedpod extends com.quasistellar.hollowdungeon.plants.Plant {
		...
		public void activate( com.quasistellar.hollowdungeon.actors.Char ch ) {
			...
			for (int i = 0; i < nSeeds && !candidates.isEmpty(); i++){
				Integer c = Random.element(candidates);
				Dungeon.level.drop(Generator.random(com.quasistellar.hollowdungeon.items.Generator.Category.SEED), c).sprite.drop(pos);
				candidates.remove(c);
			}
	...
	public static class Lotus extends NPC {
		...
		public void destroy() {
			...
			com.quasistellar.hollowdungeon.scenes.GameScene.updateFog(pos, viewDistance+1);
		}
