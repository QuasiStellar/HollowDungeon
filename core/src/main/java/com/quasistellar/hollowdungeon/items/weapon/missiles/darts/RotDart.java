...
public class RotDart extends TippedDart {
	...
	public int proc(com.quasistellar.hollowdungeon.actors.Char attacker, com.quasistellar.hollowdungeon.actors.Char defender, int damage) {
		...
		if (defender.properties().contains(Char.Property.BOSS)
				|| defender.properties().contains(com.quasistellar.hollowdungeon.actors.Char.Property.MINIBOSS)){
			Buff.affect(defender, Corrosion.class).set(5f, Dungeon.depth/3);
		} else{
			Buff.affect(defender, Corrosion.class).set(10f, com.quasistellar.hollowdungeon.Dungeon.depth);
		}
