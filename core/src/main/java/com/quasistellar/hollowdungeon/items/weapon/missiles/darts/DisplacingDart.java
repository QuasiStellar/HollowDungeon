		...
		if (!defender.properties().contains(Char.Property.IMMOVABLE)){
			...
			for (int pos = 0; pos < Dungeon.level.length(); pos++){
				if (Dungeon.level.heroFOV[pos]
						...
						&& (!Char.hasProp(defender, com.quasistellar.hollowdungeon.actors.Char.Property.LARGE) || Dungeon.level.openSpace[pos])
			...
			if (chosenDist != -1){
				...
				com.quasistellar.hollowdungeon.Dungeon.level.occupyCell(defender );
			}
