	...
	public static class CircleBack extends Buff {
		...
		public boolean act() {
			if (returnDepth == Dungeon.depth){
				left--;
				if (left <= 0){
					final com.quasistellar.hollowdungeon.actors.Char returnTarget = com.quasistellar.hollowdungeon.actors.Actor.findChar(returnPos);
					final Char target = this.target;
					com.quasistellar.hollowdungeon.sprites.MissileSprite visual = ((com.quasistellar.hollowdungeon.sprites.MissileSprite) Dungeon.hero.sprite.parent.recycle(MissileSprite.class));
					...
					visual.alpha(0f);
					float duration = com.quasistellar.hollowdungeon.Dungeon.level.trueDistance(thrownPos, returnPos) / 20f;
