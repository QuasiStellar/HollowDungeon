...
public abstract class Wand extends com.quasistellar.hollowdungeon.items.Item {
	...
	public abstract void onHit(com.quasistellar.hollowdungeon.items.weapon.melee.MagesStaff staff, com.quasistellar.hollowdungeon.actors.Char attacker, com.quasistellar.hollowdungeon.actors.Char defender, int damage);

	public boolean tryToZap(com.quasistellar.hollowdungeon.actors.hero.Hero owner, int target ){

		if (owner.buff(com.quasistellar.hollowdungeon.actors.buffs.MagicImmune.class) != null){
	...
	public void charge( com.quasistellar.hollowdungeon.actors.Char owner ) {
		if (charger == null) charger = new Charger();
		charger.attachTo( owner );
	}

	public void charge(com.quasistellar.hollowdungeon.actors.Char owner, float chargeScaleFactor ){
		charge( owner );
		charger.setScaleFactor( chargeScaleFactor );
	}

	protected void processSoulMark(com.quasistellar.hollowdungeon.actors.Char target, int chargesUsed){
		processSoulMark(target, buffedLvl(), chargesUsed);
	}

	protected static void processSoulMark(com.quasistellar.hollowdungeon.actors.Char target, int wandLevel, int chargesUsed){
		if (target != Dungeon.hero &&
				com.quasistellar.hollowdungeon.Dungeon.hero.subClass == HeroSubClass.WARLOCK &&
				//standard 1 - 0.92^x chance, plus 7%. Starts at 15%
				Random.Float() > (Math.pow(0.92f, (wandLevel*chargesUsed)+1) - 0.07f)){
			SoulMark.prolong(target, com.quasistellar.hollowdungeon.actors.buffs.SoulMark.class, com.quasistellar.hollowdungeon.actors.buffs.SoulMark.DURATION + wandLevel);
		}
	}
	...
	@Override
	public com.quasistellar.hollowdungeon.items.Item degrade() {
		super.degrade();
	...
	protected static com.quasistellar.hollowdungeon.scenes.CellSelector.Listener zapper = new  CellSelector.Listener() {
				...
				if (curWand.tryToZap(curUser, target)) {
					...
					if (curWand.cursed){
						if (!curWand.cursedKnown){
							com.quasistellar.hollowdungeon.utils.GLog.n(Messages.get(Wand.class, "curse_discover", curWand.name()));
						}
	...
	public class Charger extends com.quasistellar.hollowdungeon.actors.buffs.Buff {
		...
		private void recharge(){
			...
			com.quasistellar.hollowdungeon.actors.buffs.LockedFloor lock = target.buff(com.quasistellar.hollowdungeon.actors.buffs.LockedFloor.class);
			if (lock == null || lock.regenOn())
				partialCharge += (1f/turnsToCharge) * RingOfEnergy.wandChargeMultiplier(target);

			for (com.quasistellar.hollowdungeon.actors.buffs.Recharging bonus : target.buffs(com.quasistellar.hollowdungeon.actors.buffs.Recharging.class)){
