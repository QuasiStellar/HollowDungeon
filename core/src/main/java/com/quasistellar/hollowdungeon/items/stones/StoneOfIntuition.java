...
import com.quasistellar.hollowdungeon.windows.IconTitle;
...
public class StoneOfIntuition extends InventoryStone {
	...
	public class WndGuess extends Window {
		...
		public WndGuess(final com.quasistellar.hollowdungeon.items.Item item){
			
			com.quasistellar.hollowdungeon.windows.IconTitle titlebar = new IconTitle();
			titlebar.icon( new com.quasistellar.hollowdungeon.sprites.ItemSprite(ItemSpriteSheet.STONE_INTUITION, null) );
			...
			final com.quasistellar.hollowdungeon.ui.RedButton guess = new RedButton(""){
				@Override
				protected void onClick() {
					...
					if (item.getClass() == curGuess){
						...
						com.quasistellar.hollowdungeon.utils.GLog.n( Messages.get(WndGuess.class, "incorrect") );
					}
					curGuess = null;
					hide();
				}
			...
			HashSet<Class<?extends com.quasistellar.hollowdungeon.items.Item>> unIDed = new HashSet<>();
			final Class<?extends com.quasistellar.hollowdungeon.items.Item>[] all;

			if (item.isIdentified()){
				hide();
				return;
			} else if (item instanceof com.quasistellar.hollowdungeon.items.potions.Potion){
				unIDed.addAll(Potion.getUnknown());
				all = (Class<? extends com.quasistellar.hollowdungeon.items.Item>[]) Generator.Category.POTION.classes.clone();
				if (item instanceof com.quasistellar.hollowdungeon.items.potions.exotic.ExoticPotion){
					...
					HashSet<Class<?extends com.quasistellar.hollowdungeon.items.Item>> exoUID = new HashSet<>();
					for (Class<?extends com.quasistellar.hollowdungeon.items.Item> i : unIDed){
						exoUID.add(com.quasistellar.hollowdungeon.items.potions.exotic.ExoticPotion.regToExo.get(i));
					}
					unIDed = exoUID;
				}
			} else if (item instanceof com.quasistellar.hollowdungeon.items.scrolls.Scroll){
				unIDed.addAll(Scroll.getUnknown());
				all = (Class<? extends com.quasistellar.hollowdungeon.items.Item>[]) com.quasistellar.hollowdungeon.items.Generator.Category.SCROLL.classes.clone();
				if (item instanceof com.quasistellar.hollowdungeon.items.scrolls.exotic.ExoticScroll) {
					...
					HashSet<Class<? extends com.quasistellar.hollowdungeon.items.Item>> exoUID = new HashSet<>();
					for (Class<? extends com.quasistellar.hollowdungeon.items.Item> i : unIDed) {
						exoUID.add(com.quasistellar.hollowdungeon.items.scrolls.exotic.ExoticScroll.regToExo.get(i));
					}
					unIDed = exoUID;
				}
			} else {
			...
			for (int i = 0; i < all.length; i++){
				...
				final int j = i;
				com.quasistellar.hollowdungeon.ui.IconButton btn = new IconButton(){
					@Override
					protected void onClick() {
						curGuess = all[j];
						guess.visible = true;
				...
				im.frame(com.quasistellar.hollowdungeon.sprites.ItemSpriteSheet.Icons.film.get(Reflection.newInstance(all[i]).icon));
				im.scale.set(2f);
				btn.icon(im);
