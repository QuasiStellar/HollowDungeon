...
public class DewVial extends Item {
	...
	public void execute( final Hero hero, String action ) {
		...
		if (action.equals( AC_DRINK )) {
			...
			if (volume > 0) {
				...
				if (effect > 0) {
					hero.HP += effect;
					hero.sprite.emitter().burst( Speck.factory( com.quasistellar.hollowdungeon.effects.Speck.HEALING ), 1 + dropsNeeded/5 );
	...
	public void collectDew( Dewdrop dew ) {
		...
		if (volume >= MAX_VOLUME) {
			volume = MAX_VOLUME;
			com.quasistellar.hollowdungeon.utils.GLog.p( Messages.get(this, "full") );
		}
