	...
	public void doRead() {
		...
		for (Mob m : Dungeon.level.mobs.toArray(new Mob[0])){
			if (m instanceof PrismaticImage){
				found = true;
				m.HP = m.HT;
				m.sprite.emitter().burst(Speck.factory(com.quasistellar.hollowdungeon.effects.Speck.HEALING), 4);
			}
		}
		
		if (!found) {
			Buff.affect(com.quasistellar.hollowdungeon.items.Item.curUser, PrismaticGuard.class).set( PrismaticGuard.maxHP( Item.curUser ) );
		}
