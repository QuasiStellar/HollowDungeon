...
public class GameLog extends Component implements Signal.Listener<String> {
	...
	public synchronized void update() {
		for (String text : textsToAdd){
			...
			if (text.startsWith( GLog.HIGHLIGHT )) {
				...
				color = com.quasistellar.hollowdungeon.sprites.CharSprite.NEUTRAL;
			}
	...
	private synchronized void recreateLines() {
		for (Entry entry : entries) {
			lastEntry = com.quasistellar.hollowdungeon.scenes.PixelScene.renderTextBlock( entry.text, 6 );
	...
	@Override
	public void destroy() {
		com.quasistellar.hollowdungeon.utils.GLog.update.remove( this );
		super.destroy();
	}
