...
public class WndLangs extends Window {
	...
	public WndLangs(){
		...
		final ArrayList<com.quasistellar.hollowdungeon.messages.Languages> langs = new ArrayList<>(Arrays.asList(Languages.values()));

		com.quasistellar.hollowdungeon.messages.Languages nativeLang = Languages.matchLocale(Locale.getDefault());
		langs.remove(nativeLang);
		//move the native language to the top.
		langs.add(0, nativeLang);

		final com.quasistellar.hollowdungeon.messages.Languages currLang = Messages.lang();
		...
		if (currLang != com.quasistellar.hollowdungeon.messages.Languages.ENGLISH) add(creditsBtn);
