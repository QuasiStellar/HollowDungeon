...
public class LoopBuilder extends RegularBuilder {
	...
	public ArrayList<Room> build(ArrayList<Room> rooms) {
		...
		for (int i = 1; i < loop.size(); i++){
			...
			if (Builder.placeRoom(rooms, prev, r, targetAngle) != -1) {
				prev = r;
		...
		while (!prev.connect(entrance)){
			...
			com.quasistellar.hollowdungeon.levels.rooms.connection.ConnectionRoom c = com.quasistellar.hollowdungeon.levels.rooms.connection.ConnectionRoom.createRoom();
			if (Builder.placeRoom(loop, prev, c, Builder.angleBetweenRooms(prev, entrance)) == -1){
				return null;
			}
			loop.add(c);
			rooms.add(c);
	...
	protected float randomBranchAngle( Room r ) {
		if (loopCenter == null)
			...
			float toCenter = Builder.angleBetweenPoints( new PointF((r.left + r.right)/2f, (r.top + r.bottom)/2f), loopCenter);
