...
import com.quasistellar.hollowdungeon.levels.rooms.standard.CaveRoom;
...
public class CavesPainter extends RegularPainter {
	...
	protected void decorate(Level level, ArrayList<Room> rooms) {
		...
		for (Room room : rooms) {
			if (!(room instanceof com.quasistellar.hollowdungeon.levels.rooms.standard.EmptyRoom || room instanceof CaveRoom)) {
				continue;
			}
		...
		for (Room r : rooms) {
			if (r instanceof com.quasistellar.hollowdungeon.levels.rooms.standard.EmptyRoom) {
				for (Room n : r.neigbours) {
					if (n instanceof EmptyRoom && !r.connected.containsKey( n )) {
						...
						if (i.left == i.right && i.bottom - i.top >= 5) {
							...
							Painter.fill( level, i.left, i.top, i.width(), 1, com.quasistellar.hollowdungeon.levels.Terrain.CHASM );
						}
					}
				}
			}
