...
import com.quasistellar.hollowdungeon.actors.mobs.RotHeart;
import com.quasistellar.hollowdungeon.actors.mobs.RotLasher;
...
public class RotGardenRoom extends SpecialRoom {
	...
	private static boolean validPlantPos(com.quasistellar.hollowdungeon.levels.Level level, int pos){
	...
	private static void placePlant(Level level, int pos, Mob plant){
		...
		for(int i : PathFinder.NEIGHBOURS8) {
			if (level.map[pos + i] == Terrain.GRASS){
				Painter.set(level, pos + i, com.quasistellar.hollowdungeon.levels.Terrain.HIGH_GRASS);
			}
		}
	}
}
