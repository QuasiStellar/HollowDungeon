...
package com.quasistellar.hollowdungeon.levels.rooms;
...
public abstract class Room extends Rect implements Graph.Node, Bundlable {
	...
	public abstract void paint(com.quasistellar.hollowdungeon.levels.Level level);
	...
	public boolean canPlaceCharacter(Point p, com.quasistellar.hollowdungeon.levels.Level l){
		return inside(p);
	}
	
	public final ArrayList<Point> charPlaceablePoints(com.quasistellar.hollowdungeon.levels.Level l){
		ArrayList<Point> points = new ArrayList<>();
		for (int i = left; i <= right; i++) {
			for (int j = top; j <= bottom; j++) {
				Point p = new Point(i, j);
