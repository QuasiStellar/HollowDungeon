...
public class NewCavesBossLevel extends Level {
	...
	public void seal() {
		...
		CellEmitter.get( entrance ).start( Speck.factory( com.quasistellar.hollowdungeon.effects.Speck.ROCK ), 0.07f, 10 );
	...
	public void unseal() {
		...
		for (int j = 0; j < 5; j++){
			...
			if (Dungeon.level.heroFOV[i+j]){
				com.quasistellar.hollowdungeon.effects.CellEmitter.get(i+j).burst(BlastParticle.FACTORY, 10);
			}
		}
		GameScene.updateMap();
	...
	public void activatePylon(){
		...
		for( int i = (mainArena.top-1)*width; i <length; i++){
			if (map[i] == Terrain.INACTIVE_TRAP || map[i] == Terrain.WATER || map[i] == Terrain.SIGN){
				com.quasistellar.hollowdungeon.scenes.GameScene.add(Blob.seed(i, 1, PylonEnergy.class));
			}
		}
	...
	public String tileName( int tile ) {
		switch (tile) {
			case Terrain.GRASS:
				return Messages.get(com.quasistellar.hollowdungeon.levels.CavesLevel.class, "grass_name");
			case Terrain.HIGH_GRASS:
				return Messages.get(com.quasistellar.hollowdungeon.levels.CavesLevel.class, "high_grass_name");
			case Terrain.WATER:
				return Messages.get(com.quasistellar.hollowdungeon.levels.CavesLevel.class, "water_name");
			case Terrain.STATUE:
				//city statues are used
				return Messages.get(com.quasistellar.hollowdungeon.levels.CityLevel.class, "statue_name");
			default:
	...
	public String tileDesc( int tile ) {
		switch (tile) {
			...
			case Terrain.ENTRANCE:
				return Messages.get(com.quasistellar.hollowdungeon.levels.CavesLevel.class, "entrance_desc");
			case Terrain.EXIT:
				//city exit is used
				return Messages.get(com.quasistellar.hollowdungeon.levels.CityLevel.class, "exit_desc");
			case Terrain.HIGH_GRASS:
				return Messages.get(com.quasistellar.hollowdungeon.levels.CavesLevel.class, "high_grass_desc");
			case Terrain.WALL_DECO:
				return Messages.get(com.quasistellar.hollowdungeon.levels.CavesLevel.class, "wall_deco_desc");
			case Terrain.BOOKSHELF:
				return Messages.get(com.quasistellar.hollowdungeon.levels.CavesLevel.class, "bookshelf_desc");
			//city statues are used
			case Terrain.STATUE:
