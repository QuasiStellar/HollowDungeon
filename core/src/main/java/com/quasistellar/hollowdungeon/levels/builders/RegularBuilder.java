...
import com.quasistellar.hollowdungeon.levels.rooms.connection.ConnectionRoom;
import com.quasistellar.hollowdungeon.levels.rooms.connection.MazeConnectionRoom;
...
public abstract class RegularBuilder extends Builder {
	...
	protected void weightRooms(ArrayList<Room> rooms){
		for (Room r : rooms.toArray(new Room[0])){
			if (r instanceof com.quasistellar.hollowdungeon.levels.rooms.standard.StandardRoom){
				for (int i = 1; i < ((com.quasistellar.hollowdungeon.levels.rooms.standard.StandardRoom) r).sizeCat.connectionWeight(); i++)
					rooms.add(r);
			}
		}
	}
	...
	protected void createBranches(ArrayList<Room> rooms, ArrayList<Room> branchable,
		...
		while (i < roomsToBranch.size()){
			...
			} while( r instanceof com.quasistellar.hollowdungeon.levels.rooms.secret.SecretRoom && curr instanceof com.quasistellar.hollowdungeon.levels.rooms.connection.ConnectionRoom);
			...
			for (int j = 0; j < connectingRooms; j++){
				com.quasistellar.hollowdungeon.levels.rooms.connection.ConnectionRoom t = r instanceof SecretRoom ? new MazeConnectionRoom() : ConnectionRoom.createRoom();
				tries = 3;
