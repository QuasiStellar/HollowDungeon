...
public class AlchemyScene extends PixelScene {
		...
		IconButton btnGuide = new IconButton( new ItemSprite(ItemSpriteSheet.ALCH_PAGE, null)){
			...
			protected void onClick() {
				...
				AlchemyScene.this.addToFront(new Window(){
					...
					{
						com.quasistellar.hollowdungeon.windows.WndJournal.AlchemyTab t = new com.quasistellar.hollowdungeon.windows.WndJournal.AlchemyTab();
						int w, h;
						if (landscape()) {
							w = WndJournal.WIDTH_L; h = WndJournal.HEIGHT_L;
						} else {
							w = WndJournal.WIDTH_P; h = com.quasistellar.hollowdungeon.windows.WndJournal.HEIGHT_P;
						}
						resize(w, h);
						add(t);
						t.setRect(0, 0, w, h);
	...
	private void combine(){
		...
		if (result != null){
			...
			smokeEmitter.burst(Speck.factory( com.quasistellar.hollowdungeon.effects.Speck.WOOL ), 10 );
